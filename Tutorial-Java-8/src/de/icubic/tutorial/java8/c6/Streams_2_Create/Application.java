...
public class Application {
	...
	public static void main( String[] args ) {
		...
		{
			final IntStream stream = IntStream.iterate( 10, x -> x + 2 )
					.limit( 5 );
			stream.forEach( ( int x ) -> System.out.print( x + " " ) );
			System.out.println();
		}
		{
			final Stream<Integer> stream = Stream.iterate( 10, x -> x + 2 )
					.limit( 5 );
		...
		}
		{
			...
			final IntStream stream = IntStream.generate( () -> i.getAndIncrement() )
					.limit( 5 );
			stream.forEach( ( int x ) -> System.out.print( x + " " ) );
			System.out.println();
		}
		{
			AtomicInteger i = new AtomicInteger( 10 );
			final Stream<Integer> stream = Stream.generate( () -> i.getAndIncrement() )
					.limit( 5 );
			stream.forEach( ( Integer x ) -> System.out.print( x + " " ) );
			System.out.println();
		}
