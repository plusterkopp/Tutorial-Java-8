package de.icubic.tutorial.java8.c6.Streams_4_Terminal;
...
public class Application {
	...
	public static void main(String[] args) {
		...
		{
			final List<String> list = Arrays.asList("red", "green", "blue");
			Stream<Integer> stream = list.stream().mapToInt(s -> s.length()).mapToObj(x -> x);
			stream.forEach(x -> System.out.print(x + " "));
			System.out.println();
		}
		{
		...
		}
		{
			...
			stream.skip(1).limit(4).filter(x -> x % 2 == 0).sorted().forEach(s -> System.out.print(s + " "));
			System.out.println();
		}
		{
//			Stream<Integer> stream = Stream.of(33, 55, 44, 11, 22, 66);
